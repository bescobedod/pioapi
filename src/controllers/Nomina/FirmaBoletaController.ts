...
import {
  FirmaBoletaDtoType,
  InvalidarFirmaDtoType,
  VerificarIntegridadDtoType,
} from "../../dtos/Boleta/FirmaBoletaDto";
import { VerificarFirmaExistenteType } from "../../dtos/Boleta/VerificarFirmaExistenteDto";
...
export default class FirmaBoletaController {
  ...
  async verificarIntegridad(
    ...
    await handleSend(
      ...
      async () => {
        ...
        return {
          id_firma_boleta_pago: (req.body as VerificarIntegridadDtoType)
  ...
  async invalidarFirma(req: RequestAuth, res: Response<JsonResponse<any>>) {
    await handleSend(
      res,
      async (t) => {
        const { id_firma_boleta_pago, motivo } =
          req.body as InvalidarFirmaDtoType;
