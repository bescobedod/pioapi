...
import { ConsultaBoletaDtoType } from "../../dtos/Boleta/ConsultaBoletaDto";
import { TipoPeriodoEnum } from "../../types/PeriodosNomina/PeriodosPagadosType";

@injectable()
export default class BoletaConsultaController {
  constructor(
    @inject(BoletaConsultaService)
    private boletaConsultaService: BoletaConsultaService,
  ) {}
  ...
  async obtenerDetalleCompleto(
    ...
    await handleSend(
      ...
      "Detalle completo de pago obtenido exitosamente.",
      true,
      "NOMINA",
    );
  }
   ...
   */
  async verificarExistenciaBoleta(
    req: RequestAuth,
    res: Response<JsonResponse<any>>,
  ) {
    await handleSend(
      ...
      async () => {
        ...
        const user = req.user as userToken;
        const resultado =
          await this.boletaConsultaService.verificarExistenciaBoleta(
            Number(user.id_users), // cod_empleado del usuario autenticado
            id_periodo,
          );
