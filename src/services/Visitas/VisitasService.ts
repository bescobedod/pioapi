...
export default class VisitasService {
  constructor(
    ...
    @inject(FormSupervisionRepository)
    private formSupervisionRepository: FormSupervisionRepository,
    @inject(FileServices) private fileServices: FileServices,
    @inject(VisitaEmergenciaRepository)
    private visitaEmergenciaRepository: VisitaEmergenciaRepository,
  ) {}

  async createVisitaAndSaveFile(
    t: Transaction,
    data: CreateVisitaDtoType,
    user: userToken,
    file: CreateVisitaDtoTypeFiles,
  ): Promise<any> {
    //si el id de la visita emegencia viene en el body entonces validar si existe y editar su estado a finalizado(3)
    if (data.id_visita_emergencia)
      await this.visitaEmergenciaRepository.updateById(
        data.id_visita_emergencia,
        { id_estado: 3 },
        t,
      );
