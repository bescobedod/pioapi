...
import {
  DevolucionCreateDtoType,
  DevolucionCreateDtoTypeFiles,
...
export default class DevolucionService {
  constructor(
    ...
    @inject(DevolucionDetalleRepository)
    private devolucionDetalleRepository: DevolucionDetalleRepository,
  ) {}

  async createDevolucionService(
    ...
    return await handleTransaction(async (t) => {
      ...
      await Promise.all(
        data.detalle_devolucion.map((el) =>
          this.devolucionDetalleRepository.create(
            {
