...
import { CreateInvitadoDtoType } from "../../dtos/Convivio/PersonasConvivio/CreateInvitadoDto";
...
import { PersonasQRDtoType } from "../../dtos/Convivio/PersonasConvivio/PersonasQRDto";
...
export default class PersonasConvivioService {
  constructor(
    @inject(PersonasConvivioRepository)
    private personasConvivioRepository: PersonasConvivioRepository,
  ...
  async createInvitado(
    data: CreateInvitadoDtoType,
    user: userToken,
    t: Transaction,
  ): Promise<any> {
    const lastInvitado =
      await this.personasConvivioRepository.findLastPersonInvitado(true);
  ...
  async findOrCreatePersonaConvivio(
    data: PersonasQRDtoType,
    t: Transaction,
  ): Promise<any> {
    let personaConvivio =
      await this.personasConvivioRepository.findPersonaByCodigoAndIdTipo(
        data.codigo,
    ...
    if (!personaConvivio)
      throw new Error(`Error al encontrar la persona del convivio`);
