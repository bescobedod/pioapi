...
export default class EntradaPdvService {
  constructor(
    @inject(EntradaInventarioDetalleRepository)
    private entradaInventarioDetalleRepository: EntradaInventarioDetalleRepository,
  ...
  validSerieEntrada(data: saveRecepcionDtoType) {
    ...
    const objValidSeries = [...SERIES_AVICOLA, ...SERIES_INSUMOS];
  ...
  async createEntradas(
  ...
  ): Promise<tEntradaInventarioModel | null> {
    const result = await handleTransaction(async (t) => {
      ...
      let createEncabezadoEntradaInventario: tEntradaInventarioModel | null =
        null;

      createEncabezadoEntradaInventario =
      ...
      if (createEncabezadoEntradaInventario)
        return createEncabezadoEntradaInventario;

      createEncabezadoEntradaInventario =
        await this.entradaInventarioRepository.create(
          {
            ...dataCreateEntradaInventario,
      ...
      const idEntradaInventario: number | null =
        createEncabezadoEntradaInventario?.idEntradaInventario || null;

      if (!idEntradaInventario)
        throw new Error("Error al obtener el [idEntradaInventario]");

      await Promise.all(
        detalle.map((el) =>
          this.entradaInventarioDetalleRepository.create(
            {
  ...
  async updateEncabezadoByIdEntradaInventario(
    entrada: tEntradaInventarioModel,
    responseSap: ResponseEntryArticulosSapType,
  ): Promise<number> {
    const result: number = await handleTransaction(async (t) => {
      const updateCount: number =
        await this.entradaInventarioRepository.updateByIdEntradaInventario(
