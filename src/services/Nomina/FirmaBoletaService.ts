...
import { FirmaBoletaDtoType } from "../../dtos/Boleta/FirmaBoletaDto";
...
import {
  generarDatosBoletaQuincena,
  hashBoleta,
  orderDataPlanillaQuincena,
} from "../../utils/Boletas/BoletasUtil";
...
export default class FirmaBoletaService {
  constructor(
    @inject(FirmaBoletaPagoRepository)
    private firmaBoletaPagoRepository: FirmaBoletaPagoRepository,
    @inject(tPlanillaRepository)
    private tPlanillaRepository: tPlanillaRepository,
    @inject(tPeriodoRepository) private tPeriodoRepository: tPeriodoRepository,
    @inject(tFirmaBoletaRepository)
    private tFirmaBoletaRepository: tFirmaBoletaRepository,
    @inject(tPeriodoEspecialBoletaRepository)
    private periodoEspecialBoletaRepository: tPeriodoEspecialBoletaRepository,
    @inject(Bono14Repository) private bono14Repository: Bono14Repository,
    @inject(AguinaldoRepository)
    private aguinaldoRepository: AguinaldoRepository,
    @inject(PeriodoVacacionRepository)
    private periodoVacacionRepository: PeriodoVacacionRepository,
    @inject(tVacacionRepository)
    private vacacionRepository: tVacacionRepository,
  ) {}
  ...
  async firmarBoleta(
    ...
    try {
      ...
      if (isQuincena)
        periodo = await this.tPeriodoRepository.findById(data.id_periodo, true);
      if (isAguinaldo || isBono14)
        periodo = await this.periodoEspecialBoletaRepository.find(
          data.id_periodo,
          false,
          true,
        );
      if (isVacacion)
        periodo = await this.periodoVacacionRepository.findById(
          data.id_periodo,
          true,
        );
      ...
      if (!periodo.pagada)
        throw new Error(
          "Solo se pueden firmar boletas de períodos que ya han sido pagados.",
        );
      ...
      if (isQuincena)
        planilla = await this.tPlanillaRepository.findByEmpleadoAndPeriodo(
          Number(user.id_users),
          data.id_periodo,
          true,
        );
      if (isBono14)
        planilla = await this.bono14Repository.findByYearAndUser(
          year,
          Number(user.id_users),
          false,
          true,
      ...
      if (isAguinaldo)
        planilla = await this.aguinaldoRepository.findByYearAndUser(
          year,
          Number(user.id_users),
          false,
          true,
      ...
      if (isVacacion)
        planilla = await this.vacacionRepository.findPlanillaVacacion(
          periodo?.idPeriodo ?? 0,
          false,
          true,
        );
      ...
      if (!planilla)
        throw new Error(
          "No se encontró planilla para este empleado en el período especificado.",
        );

      //datos para firma pioapp
      const dataHash = isQuincena
        ? orderDataPlanillaQuincena(planilla)
        : planilla;
      const hash = hashBoleta(dataHash);
      //datos para firma pdv
      const firmaUuid = uuidv4();
      const datosBoleta = isQuincena
        ? generarDatosBoletaQuincena(planilla, periodo)
        : { ...planilla, ...periodo };
      const datosBoletaBase64 = Buffer.from(
        JSON.stringify(datosBoleta),
      ).toString("base64");
      const periodoPdv = {
      ...
      const firmaPdv =
        await this.tFirmaBoletaRepository.findOrCreateByYearAndTipo(
          year,
          data?.tipo || TipoPeriodoEnum.QUINCENA,
          Number(user.id_users),
          periodoPdv[data.tipo as TipoPeriodoEnum],
          {
            empresa: "0000",
            tienda: "0000",
            ...(isQuincena
  ...
  async obtenerHistorialFirmas(user: userToken): Promise<any[]> {
    const firmas = await this.firmaBoletaPagoRepository.findByUserId(
      Number(user.id_users),
      true,
    );

    const historialPromises = firmas.map(async (firma) => {
      const periodo = await this.tPeriodoRepository.findById(
        firma.id_periodo,
        true,
      );
  ...
  async verificarIntegridad(id_firma_boleta_pago: string): Promise<boolean> {
    const firma =
      await this.firmaBoletaPagoRepository.findById(id_firma_boleta_pago);
    if (!firma) {
  ...
  async verificarFirmaExistente(
    ...
    id_periodo: number,
    tipo: number,
  ): Promise<VerificarFirmaExistenteResponse> {
    // Buscar firma en la base de datos principal
    const firmaExistente =
      await this.firmaBoletaPagoRepository.findByUserAndPeriodo(
        id_users,
        id_periodo,
        true,
        false,
