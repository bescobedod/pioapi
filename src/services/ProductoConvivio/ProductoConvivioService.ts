...
export default class ProductoConvivioService {
  constructor(
    @inject(ProductoConvivioRepository)
    private productoConvivioRepository: ProductoConvivioRepository,
  ) {}

  async createNwProductoConvivio(
    data: NwProductConvivioDtoType,
    user: userToken,
    t: Transaction,
  ): Promise<any> {
    const createProductoConvivio = await this.productoConvivioRepository.create(
      {
        ...
        descripcion_producto_convivio:
          data?.descripcion_producto_convivio ?? null,
