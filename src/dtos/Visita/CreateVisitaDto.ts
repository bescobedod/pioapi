...
export const CreateVisitaDto = yup
  .object({
    ...
    tienda_nombre: yup
      .string()
      .required("la [tienda_nombre] es un campo obligatorio."),
    tienda_direccion: yup.string(),
    id_tipo_visita: yup
      .number()
      .integer()
      .required("el [id_tipo_visita] es un campo obligatorio."),
    ...
    uso_uniforme: yup
      ...
      .when("id_tipo_visita", {
        is: 1,
        then: (schema) =>
          schema.required("El [uso_uniforme] es un campo obligatorio."),
    ...
    buzon_cerrado: yup
      ...
      .when("id_tipo_visita", {
        is: 1,
        then: (schema) =>
          schema.required("El [buzon_cerrado] es un campo obligatorio."),
        otherwise: (schema) => schema,
      }),
    tienda_limpia: yup
      .boolean()
      .when("id_tipo_visita", {
        is: 1,
        then: (schema) =>
          schema.required("El [tienda_limpia] es un campo obligatorio."),
        otherwise: (schema) => schema,
      }),
    cantidad_personas: yup
      .boolean()
      .when("id_tipo_visita", {
        is: 1,
        then: (schema) =>
          schema.required("El [cantidad_personas] es un campo obligatorio."),
        otherwise: (schema) => schema,
      }),
    cantidad: yup.number(),
