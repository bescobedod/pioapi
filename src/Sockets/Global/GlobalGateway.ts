...
export default class GlobalGateway implements ISocket {
  ...
  constructor(
    ...
    @inject(NotificacionAppRepository)
    private notificacionAppRepository: NotificacionAppRepository,
  ) {
  ...
  init(): void {
    ...
    NotificationEventEmitter.on(
      "notificacion-leida",
      (notificacion: NotificacionAppModel) => {
        const roomUser = `global_room_${notificacion.id_users}`;
        this.socketService.emitByRoom(
          roomUser,
          "leida-notificacion",
          notificacion,
        );
    ...
    RecepcionEventEmitter.on(
      EMIT_NOTIFICATION,
      async (body: ParamsEventNotificactionEventType) => {
    ...
    this.socketService.connection((socket: SocketAuthType) => {
      ...
      this.socketService.onEvent(
        ...
        async (data) => {
          const countNotificaciones =
            await this.notificacionAppRepository.count({
              id_users: Number(socket.user?.id_users ?? 0),
              leido: false,
            });
          this.socketService.emitByRoom(
            roomUser,
            "notificaciones-list-count",
            countNotificaciones,
          );
        },
      );
